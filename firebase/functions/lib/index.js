...
exports.onWaterLogVerified = exports.onTaskVerified = exports.waterReminderEmail = exports.taskReminderEmail = exports.onEmergencyAlert = void 0;
...
async function getMemberEmails(targetUserIds) {
    let query = db.collection("users").where("role", "==", "member");
    ...
    snap.forEach((doc) => {
        ...
        if (!data.email)
            return;
        if (targetUserIds && targetUserIds.length > 0 && !targetUserIds.includes(doc.id))
            return;
        members.push({ name: (_a = data.name) !== null && _a !== void 0 ? _a : "Member", email: data.email });
    });
    return members;
}
/** Branded HTML email wrapper */
function htmlWrap(title, body, color = "#e879a0") {
    return `
<!DOCTYPE html>
<html>
<head>
...
exports.onEmergencyAlert = functions.firestore
    ...
    if (!alert || !alert.isActive)
        return;
    const targetUserIds = ((_a = alert.targetUserIds) === null || _a === void 0 ? void 0 : _a.length)
        ? alert.targetUserIds
        : undefined;
    ...
    const sends = members.map(({ name, email }) => transporter.sendMail({
...
exports.taskReminderEmail = functions.pubsub
    ...
    tasksSnap.forEach((doc) => {
        ...
        if (!t.userId)
            return;
        if (!tasksByUser[t.userId])
            tasksByUser[t.userId] = [];
    ...
    for (const [userId, tasks] of Object.entries(tasksByUser)) {
        ...
        <p style="color:#6b7280;font-size:14px;margin:0 0 16px;">
          Hi ${(_a = userData.name) !== null && _a !== void 0 ? _a : "Member"}, you have ${tasks.length} task${tasks.length > 1 ? "s" : ""} due soon. Don't forget to complete and submit!
        </p>
        ...
        }).catch((err) => console.error(`Task email failed for ${userId}:`, err)));
    }
    await Promise.all(sends);
...
exports.waterReminderEmail = functions.pubsub
    ...
    for (const userDoc of usersSnap.docs) {
        ...
        if (!userData.email)
            continue;
        if (userData.isNotificationsOn === false)
            continue;
        ...
        const body = `
        <p style="color:#6b7280;font-size:14px;margin:0 0 16px;">
          Hi ${(_a = userData.name) !== null && _a !== void 0 ? _a : "Member"}, time for a hydration check! 💧
        </p>
        ...
        }).catch((err) => console.error(`Water email failed for ${userDoc.id}:`, err)));
    }
    await Promise.all(sends);
...
exports.onTaskVerified = functions.firestore
    ...
    if (before.verificationStatus === after.verificationStatus)
        return;
    if (after.verificationStatus === "pending_review")
        return;
    ...
    const body = `
      <p style="color:#6b7280;font-size:14px;margin:0 0 16px;">
        Hi ${(_a = userData.name) !== null && _a !== void 0 ? _a : "Member"}, your task submission has been reviewed.
      </p>
      <div style="background:${isVerified ? "#f0fdf4" : "#fff1f2"};border-left:4px solid ${isVerified ? "#22c55e" : "#dc2626"};border-radius:8px;padding:16px 20px;margin-bottom:16px;">
        <div style="font-weight:700;color:#1a1a2e;font-size:15px;margin-bottom:4px;">${(_b = after.taskTitle) !== null && _b !== void 0 ? _b : "Task"}</div>
    ...
    await transporter.sendMail({
        ...
        to: userData.email,
        subject: isVerified
            ? `✅ Task Verified — ${(_c = after.taskTitle) !== null && _c !== void 0 ? _c : "Task"}`
            : `❌ Task Rejected — ${(_d = after.taskTitle) !== null && _d !== void 0 ? _d : "Task"}`,
...
exports.onWaterLogVerified = functions.firestore
    ...
    const body = `
      <p style="color:#6b7280;font-size:14px;margin:0 0 16px;">
        Hi ${(_a = userData.name) !== null && _a !== void 0 ? _a : "Member"}, your hydration log has been reviewed.
      </p>
